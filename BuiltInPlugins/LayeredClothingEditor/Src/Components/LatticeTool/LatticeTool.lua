...
function LatticeTool:renderLattices()
	...
	local makeLattice = props.MakeLattice
	...
	local selectControlPoint = props.SelectControlPoint
	...
	if boundsData then
		for deformer, bounds in pairs(boundsData[editingCage]) do
			lattices[deformer] = Roact.createElement(Lattice, {
				...
				SelectLatticeControlPoint = selectControlPoint
			})
		end
	end
	return lattices
...
local function mapDispatchToProps(dispatch)
	return {
		MakeLattice = function(deformer)
			dispatch(MakeLattice(deformer))
		end,
		SelectControlPoint = function(id, gridPosition, deformer, multiselect)
			dispatch(SelectControlPoint(id, gridPosition, deformer, multiselect))
		end,
